...
<div class="page-header-sl">
	...
	<div class="button-box">
		...
		<button type="button" class="btn btn-primary btn-xs" onclick="downLoadTemp('员工工时批量录入模板.xls')"> 模板下载</button>
...
function downLoadTemp(fileName){
	...
	document.forms[0].action = "<%=request.getContextPath() %>/staffWorkingHourManage/download_excel_temp?ran="+ran;
...
function uploadProFile() {
	...
	if(checkResult){
		loadPage("open");
		var ran = Math.random()*1000;
		document.forms[0].action ="<%=request.getContextPath() %>/staffWorkingHourManage/readWorkHourExcel?&ran="+ran;
		document.forms[0].submit();
		loadPage("close");
	}
}
...
function downLoadErr(_this){
	...
	$("#fileName").val(fileName);
	document.forms[0].action = "<%=request.getContextPath() %>/staffWorkingHourManage/downloadErrExecl?ran="+ran;
	document.forms[0].submit();
	loadPage("close");
}
function queryList(){
