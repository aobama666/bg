...
<div class="page-header-sl">
	...
	<div class="button-box">
		<button type="button" class="btn btn-danger btn-xs" style="display:none;" id="proErrInfoButton" onclick="downLoadErr(this)"> 项目错误信息下载</button>
		<button type="button" class="btn btn-danger btn-xs" style="display:none;" id="empErrInfoButton" onclick="downLoadErr(this)"> 人员错误信息下载</button>
		<button type="button" class="btn btn-primary btn-xs" onclick="downLoadTemp('项目信息模板.xls')"> 项目信息模板下载</button>
		<button type="button" class="btn btn-primary btn-xs" onclick="downLoadTemp('参与人员模板.xls')"> 参与人员模板下载</button>
...
<div class="form-box">
	<form action="" id="form1" name="form1" encType="multipart/form-data" method="post" target="hidden_frame">
		...
		<div class="form-group col-xs-12">
			<label for="empName" style="width:110px;margin-top:5px">项目信息文件：</label>
	...
	<form action="" id="form2" name="form2" encType="multipart/form-data" method="post" target="hidden_frame">
		<div class="form-group col-xs-12">
			<label for="empName" style="width:110px;margin-top:5px">参与人员信息文件：</label>
			<div class="controls" style="margin-left:110px">
				<input id="file" type="file" name="empFile" property="empFile" style="display:inline-block;width:395px">
				<button type="button" class="btn btn-success btn-xs" onclick="uploadEmpFile()"> 确定</button>
			</div>
		</div>
	</form>
...
function downLoadTemp(fileName){
	...
	document.forms[0].action = "<%=request.getContextPath() %>/project/download_excel_temp?ran="+ran;
...
function uploadProFile() {
	...
	if(checkResult){
		...
		document.forms[0].action ="<%=request.getContextPath() %>/project/readProExcel?&ran="+ran;
		document.forms[0].submit();
		loadPage("close");
	}
}
