var roomAddInfoCommon = {
		...
		getFormDataInfo:function(e){
			var formData = {};
			if(!e){
				e = $(".input_div>input,.input_div>select,.input_div>textarea");
			}
...
var dataForm = {
		...
		validNullable:function(e){
			...
			for(var a = 0;a<e.length;a++){
				...
				if(!eleValue){
					...
					ele.removeClass("validPass").addClass("validRefuse");
					return false;
				}
			}
			return true;
	    ...
	    checkEmail:function(){
			var mailobj = $(".checkemail")
			for(var i=0;i<mailobj.length;i++){
				if($(mailobj[i]).val().indexOf("@")==-1){
					messager.tip('请输入正确的邮箱格式',2000);
                    $(mailobj[i]).removeClass("validPass").addClass("validRefuse");
                    return false;
				}
			}
			return true;
