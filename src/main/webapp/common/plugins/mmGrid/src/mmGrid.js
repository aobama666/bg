    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el){
            ...
            if(opts.checkCol){
                ...
                opts.cols.unshift({title:chkHtml,width: opts.checkColWidth, align: 'center' ,lockWidth: true, checkCol: true, renderer:function(){
        ...
        , _initEvents: function(){
            ...
            $body.on('click','td',function(e){
                ...
                //判断如果是可编辑列，则不做选中处理
                if(opts.cosEdit!=undefined){
                	var cols=opts.cosEdit.split(",");
                	var preAll=$(this).prevAll();
                	for(var i=0;i<cols.length;i++){
                		if(preAll.length==cols[i]){
                			return;
                		}
                	}
                }
        ...
        , selectedRowsByName: function(name){
        	var $body = this.$body;
        	var selected = "";
