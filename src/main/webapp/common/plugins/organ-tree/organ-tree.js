$.fn.extend({
	organTree:function (properties) {
		return this.each(function () {
			var organBox = $(this);
			var organCode,organName,root,iframe,ct,limit,level,type,func;
			...
			type = properties.type == undefined ? "" : properties.type;//数据来源：type=rz 人资专用，其他为报工默认
			...
			organBox.find("#"+bindLayId+",span>span,input[name="+organName+"]").off().click(function(){
				...
				var p = {
						...
						content:[basePath+'/organstufftree/initOrganTree?root='
						         ...
						         +'&popEvent='+popEvent+'&iframeId='+iframeId+'&iframe='+iframe+'&type='+type+'&func='+func,'no'],
