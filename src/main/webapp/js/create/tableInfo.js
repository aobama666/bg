...
$("#datagrid").datagrid({
    url:'/bg/one/selectForTable',
    form:'#queryForm',
    columns:[
        {name: '序号', data: 'ROW_ID'},
        {name:'选项',data:'ROW_ID',checkbox:true,style:{width:'3%'},forMat:function(row){
       	 globArray[tindex]=row;
			 return '<input type="checkbox" name="oneCheck" value="'+(tindex++)+'" title="'+row.COLUMN_NAME+'"/>';
	 	    }
        },
        {name:'字段名称',data:'COLUMN_NAME'},
        {name:'字段类型',data:'DATA_TYPE'},
        {name:'字段长度',data:'DATA_LENGTH'},
        {name:'是否为空',data:'NULLABLE'},
        {name:'字段描述',data:'COMMENTS'},
        {name:'主键',data:'KEY'}
...
function appendPeople(){
    if (endEditings()){
        $('#joinPeople').datagrid('appendRow',{WORK_MONTH:'0',MATERIAL_ID:$("#materialId").val(),PRJ_ID:$("#projectId").val(),USER_ID:'',TEL_PHONE:'',MOBILE_PHONE:''});
        editIndex = $('#joinPeople').datagrid('getRows').length-1;
        $('#joinPeople').datagrid('beginEdit', editIndex);
    }
}
/**  弹框layer 
 * layer.open(object)  
