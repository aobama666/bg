(function ($, window, document, undefined) {
    ...
    ComboTree.prototype.initevent = function () {
        ...
        this.comboxinput.on('keyup', function (event) {
            ...
            var lis = _this.selectul.find('li'),
                targetli = null,
                _tempattr = null;
            ...
            function getChildren(parent) {
                var lichild = parent.children('li');

                if (lichild.length) {
                    for (var i = 0, _p = lichild, len = _p.length; i < len; i++) {
                        ...
                        var subulcontainer = targetli.find('>.tree-sub-body');

                        if (subulcontainer.length > 0) {
                            var _tempul = subulcontainer.find('>ul');
                            getChildren(_tempul);
                        }
                    }
                }
            ...
            if (val.trim() !== '') {
                var lis1 = _this.selectul.find('li');
                lis1.each(function (index, item) {
                    var _item = $(item);
                    var matched = _item.find('li[matched="matched"]');
                    if (_item.length === 0) {
                    ...
                    if (matched.length === 0) {
                        if (_item.attr('matched') === 'matched') { //如果当前元素匹配，则保留当前的删除它后面的所有元素
                            ...
                            var _matcheditem = _item.find('>div.tree-sub-body');
                ...
                var children = _this.comboxulcontainer.find('li[matched=matched]');
    ...
    ComboTree.prototype.checkrow = function (target, oliId, ischeckbox) {
        ...
        var item = $("<span data-id='" + oliId + "' class='input-keyword-item'></span>"),
