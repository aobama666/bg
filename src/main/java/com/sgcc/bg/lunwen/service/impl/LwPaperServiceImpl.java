...
import com.sgcc.bg.lunwen.controller.LwPaperController;
...
//            List<String> list=lwSpecialistMapper.getEmail();
            ...
            if (errorList.size() > 0) {
                ...
                Object[][] title = {
                        ...
                        { "作者", "author","nowrap" },
                        { "单位", "unit","nowrap" },
                        { "期刊名称","unitNature","nowrap"},
                        { "推荐单位","position","nowrap"},
                        { "被引量","researchDirection","nowrap"},
                        { "下载量","field","nowrap"},
                        { "领域","field","nowrap"},
                        { "错误说明","errInfo"}
                };
        ...
        if(!lwPaperList.isEmpty()) {
            //入库
//            lwPaperMapper.addList(lwSpecialistList);
        }
