...
public class StaffWorkingHourManageServiceImpl implements IStaffWorkingHourManageService {
	 ...
	 */
	@Override
	public String getLimitDeptIds(String username){
		String limit="";
		List<Map<String, Object>> organList=organStuffTreeService.queryDeptByCurrentUserPriv("41000001", "2", username);
	...
	public List<Map<String, String>> searchForWorkHourInfo(String startDate, String endDate, String deptCode,
		...
		String deptIds=getLimitDeptIds(webUtils.getUsername());
		smServiceLog.info("得到的权限部门ids： "+deptIds);
		List<Map<String, String>> list=smMapper.getWorkHourInfoByCondition(
				deptIds,startDate,endDate, deptCode,category,proName,empName,status,start,end);
		return list;
	}
	...
	public int getItemCount(String startDate, String endDate, String deptCode, String category, String proName,
		...
		String deptIds=getLimitDeptIds(webUtils.getUsername());
		return smMapper.getItemCount(deptIds,startDate, endDate, deptCode, category, proName, empName, status);
	}
				...
				if (!"#N/A!#N/A!".equals(checkStr.toString()) && !"".equals(checkStr.toString())) {// 校验此行是否为空
					...
					if(cellValue[8] == null || "".equals(cellValue[8])){
						...
						if(worker==null){
							...
							String limitDdptIds=getLimitDeptIds(webUtils.getUsername());//当前专责有权限部门id
	...
	public String exportSelectedItems(String ids, HttpServletResponse response) {
		...
		if(ids.indexOf("{")!=-1){
			...
			dataList=smMapper.getWorkHourInfoByCondition(
					getLimitDeptIds(webUtils.getUsername()),startDate,endDate, deptCode,category,proName,empName,hrCode,"0","1000000");
		}else{
